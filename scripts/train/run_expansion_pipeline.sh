...
uv run python scripts/jsonl_tool.py duplicate \
    --input seeds/v03/expansion_seeds.jsonl \
    --output seeds/v03/expansion_seeds_x1.jsonl \
    --copies 1

echo "2. AIify..."
uv run arka --config configs/v03/01-aiify-expansion.yaml --run-id expansion-aiify
...
echo "5. Humanize..."
uv run arka --config configs/v03/02-humanize-expansion.yaml --run-id expansion-humanize
