...
Run::
  ...
  rtk uv run python scripts/figures/plot_ablations.py
"""
...
ABLATION_GROUPS = {
    "reward": {
        "A0  current_components": ("outputs/ablations/gemma4-rl-ablation-a0-current-components_summary.json", "#6366f1"),
        "A1  scalar_current":     ("outputs/ablations/gemma4-rl-ablation-a1-scalar-current_summary.json",     "#0ea5e9"),
        "A2  scalar_softened":    ("outputs/ablations/gemma4-rl-ablation-a2-scalar-softened_summary.json",    "#f59e0b"),
    },
    "stability": {
        "B0  G=8  acc=8  LR=2e-5":  ("outputs/ablations/gemma4-rl-ablation-b0-g8-acc8-lr2e5_summary.json",   "#6366f1"),
        "B1  G=16 acc=16 LR=1e-5":  ("outputs/ablations/gemma4-rl-ablation-b1-g16-acc16-lr1e5_summary.json", "#0ea5e9"),
        "B2  G=16 acc=16 LR=2e-5":  ("outputs/ablations/gemma4-rl-ablation-b2-g16-acc16-lr2e5_summary.json", "#f59e0b"),
    },
    "scaling": {
        "C0  scale=group": ("outputs/ablations/gemma4-rl-ablation-c0-scale-group_summary.json", "#6366f1"),
        "C1  scale=batch": ("outputs/ablations/gemma4-rl-ablation-c1-scale-batch_summary.json", "#0ea5e9"),
        "C2  scale=none":  ("outputs/ablations/gemma4-rl-ablation-c2-scale-none_summary.json",  "#f59e0b"),
    },
}
...
def _panel(ax, runs: dict, key: str, title: str, ylabel: str, *, smooth_raw_too: bool = False) -> None:
...
@click.command()
@click.option(
    "--output-dir", default="outputs/figures/ablations",
    show_default=True, type=click.Path(file_okay=False, path_type=Path),
)
@click.option(
    "--group", default="reward", show_default=True,
...
def main(output_dir: Path, group: str) -> None:
    ...
    _panel(fig.add_subplot(gs[0, 0]), runs, "reward",
           "Reward (EMA-20, raw shaded)", "reward", smooth_raw_too=True)
    _panel(fig.add_subplot(gs[0, 1]), runs, "reward_std",
           "Reward std (EMA-20)", "reward_std")
    _panel(fig.add_subplot(gs[1, 0]), runs, "grad_norm",
           "grad_norm (EMA-20)", "grad_norm")
    _panel(fig.add_subplot(gs[1, 1]), runs, "completions/mean_length",
           "completions/mean_length (EMA-20)", "tokens")

    fig.suptitle(titles[group], fontsize=14, fontweight="bold", color=COLOR_TEXT, y=0.995)
